...
def build_map(map_size: tuple[int, int], walls: int, droids: tuple[int, ...]):
    ...
    PAREDES = [f"wall_{i}" for i in range(walls)]
    # Restricciones:
    ...
    DROIDES = [f"droid_{i}" for i in range(len(droids))]
    # Restricciones:
    ...
    total_domain = [(row, col) for row in range(map_x) for col in range(map_y)]

    no_borders_domain = [
        (row, col) for row in range(1, map_x - 1) for col in range(1, map_y - 1)
    ]
